...
load_samples <- function(){
  ## load id harz
  harz_id <- read.table("P:/timesync/bb/tsync_plots_bb.csv", 
                      sep = ",", dec = ".", header = TRUE)
...
extract_fun <- function(tile) {
  ...
  if (nrow_points_tile > 0 ){ ## there are smps 
    ...
    for(index in index_list){
      ...
      index_file <- paste0(cube_dir,tile,"/1985-2021_001-365_HL_TSA_LNDLG_",index, "_TSS.tif")
      ...
      # ## store 
      write.table(smps_tile, paste0(out_dir,tile,"_",index,"_extracted.csv"), sep = ";", dec = ".",
