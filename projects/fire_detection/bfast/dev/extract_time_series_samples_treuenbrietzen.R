...
ids <- load_samples()
ids <- read.csv2("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/treuenbrietzen_coordinates.csv", sep = ",")
...
extract_fun <- function(tile) {
  ...
  if (nrow_points_tile > 0 ){ ## there are smps 
    ...
    for(index in index_list){
      ...
      index_file <- paste0("A:/01_Data_Level3/202203_LANDSAT_S2/",tile,"/1985-2021_001-365_HL_TSA_LNDLG_",index, "_TSS.tif")
      ...
      smps_tile <- cbind(ids$x, ids$y, ids$plotid,
                         terra::extract(index_stack, cbind(ids$x,ids$y)))
