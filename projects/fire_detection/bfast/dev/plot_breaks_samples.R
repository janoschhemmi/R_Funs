...
get_sensor <- function(names) {
  sensor      <- substr(names, 11, 13)
  sensor[sensor=="SEN"] <- "Sentinel-2"
  sensor[sensor=="LND"] <- "Landsat"
  ...
  for(number_detected_break in seq(1:n_bps)){
    ...
    for (Index__ in index_list){
      ...
      if (length(nonna_list) != 0){
        ...
        if(anyDuplicated(time(dts_index))){
          print("catched duplicated dates 4x")
          index(dts_index)[duplicated(date_decimal(time(dts_index)))] <- index(dts_index)[duplicated(date_decimal(time(dts_index)))] + 1/365
        }
        ...
        i#qr_pre_breakyear         <- quantiles_pre_break_year[4]-quantiles_pre_break_year[2] 
...
do_fun_at_ref_smps <- function(x, dates,original_dates, dates_of_validation , disturbance_of_validation, 
  ...
  if (corrupt == 0){ ## meaning not only NA in data
    ...
    if (is.na(bp$breakpoints[1]) || (class(bp)[1]=="try-error")) { ## if no breakpoints detected or 
      ...
      ## create tile folder
      ifelse(!dir.exists(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/", "plots_",output_folder_name,"_no_breaks_detected")),
             dir.create(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/", "plots_", output_folder_name,"_no_breaks_detected")), FALSE)
      ifelse(!dir.exists(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/", "plots_",output_folder_name,"_no_breaks_detected/",tile)),
             dir.create(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/", "plots_", output_folder_name,"_no_breaks_detected/",tile)), FALSE)
      ## 
      
      print(paste0( smp_id, "_plot_",Index_,"_strucchange",additional,".png" ))
      print(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/","plots_",output_folder_name,"_no_breaks_detected/",tile))
      ggsave(filename = paste0( smp_id, "_plot_",Index_,"_strucchange",additional,".png" ),
             path = paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/","plots_",output_folder_name,"_no_breaks_detected/",tile),
             dpi = 150, width = 10, height = 5)
      saveRDS(out.plot, file =paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/","plots_",output_folder_name,"_no_breaks_detected/",tile,"/", smp_id, "_plot_",Index_,"_strucchange",additional,"",additional_title,".rds" ) )
      ...
      if (!is.na(bp$breakpoints[1]) && (corrupt ==  0)){
        ...
        # if(file.exists(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/break_detection_tables/",output_folder_name,tile,output_name,".csv" ))){
        #   
        #   ## load file 
        #   smp_before <- read.table(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/break_detection_tables/",output_folder_name,tile,output_name ,".csv"),
        #                            sep =";", dec =".", header = TRUE)
        #   ## join 
        #   smp_data_out <- as.data.frame(rbind(smp_before, smp_data_out))
        # }
        
        # write.table(smp_data_out, paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/break_detection_tables/",output_folder_name,tile,output_name,".csv" ), sep =";", dec = ".", col.names = TRUE, row.names = FALSE)
        ...
        if((plot == 1) == TRUE)  {
          ...
          ## create tile folder 
          ifelse(!dir.exists(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/plots_",output_folder_name)),
                 dir.create(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/plots_",output_folder_name)), FALSE)
          
          ifelse(!dir.exists(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/plots_",output_folder_name,tile,"/")),
                 dir.create(paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/plots_",output_folder_name,tile,"/")), FALSE)
          ## safe plot # smp_id 2112
          ggsave(filename = paste0( smp_id, "_plot_",Index_,"_strucchange",additional,"_",additional_title,".png" ), path = paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/plots_",output_folder_name,tile,"/"),
                 dpi = 300, width = 10, height = 5)
          print("safe plot")
          saveRDS(out.plot, file =paste0("P:/workspace/jan/fire_detection/plots/LPS/breaks_ts/","plots_",output_folder_name,tile,"/", smp_id, "_plot_",Index_,"_strucchange",additional,"",additional_title,".rds" ) )
...
main_fun  <- function(tile){
  ...
  Index_ <- "NBR"
  df_file <- paste0("P:/workspace/jan/fire_detection/Landsat_ts/extracted_Landsat_Sentinel2_ts_with_outliers_till_2022_post2/",tile,"_",Index_,"_extracted.csv")
  print("------------------------------")
  ...
  if(data_excists_in_tile == 1){
    ...
    ## add sensor 
    df_ts_long$sensor <- get_sensor(df_ts_long$Force_name) 
...
cube_dir <- "A:/01_Data_Level3/2021_S2_10m//"
...
# 
# ## Output folder names
output_name         <- "_output_extracted_data_h40_till_2022_S2_nbr"
output_folder_name  <- "NBR_S2_h40_till_2022_one_year_added/"
...
list_tiles <-list_tiles[!list_tiles %in% c("X0069_Y0041","X0071_Y0043","X0068_Y0045")]
# tile <- "X0069_Y0041"

system.time(
  foreach (ooo  = 1:52, .combine = rbind) %do% { ## 54
