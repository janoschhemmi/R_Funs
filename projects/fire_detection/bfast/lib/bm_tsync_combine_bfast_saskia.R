...
bm_tsync_combine_bfast <- function(ref_file, metrics_file, out_csv_file, exclude_plots=NULL, exclude_years=NULL, overwrite=F) {
  ...
  undisturbed <- dplyr::filter(refs, .data$disturbance==0) %>%
    dplyr::filter(!.data$plotid %in% unique(disturbed$plotid)) %>% ## exept ids with disturbance
  ...
  # check plots with multiple disturbances
  #refs$plotid[refs$dist_id > 5]
  #refs[refs$plotid==3332,]
  # refs[refs$plotid==700,]
  
  
  # create disturbance by vertex matrices
  # for each plot id get ref dates 
