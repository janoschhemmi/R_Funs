...
"P:/"
model_spec_int         <- readRDS("01_model_spec_int.rds")
...
result_02_int_model_spec         <- aa_int(model_spec_int ,df_features$int_paper, df_features$int_split)


## oa int old bands 2018 2019 0.964493 0.001559
## oa split 2018 2019 475  0.959132 0.002272
## oa split 2019  400 features  [1] 0.951285 0.002138
...
#                     "01_04_cm_model_int_original.csv")
...
result_02_int_model_spec ##  0.693719920 0.007462735
...
aaStats_2 <- result_02_int_model_spec$stats
...
p_spec <- ggplot2::ggplot(result, aes(class, accuracy, fill=src)) + 
  ...
  scale_fill +
  ggtitle("Model trained on age-split level") +
...
p_dif <- ggplot2::ggplot(result_2_env, aes(class, accuracy, fill=src)) + 
  ggplot2::geom_bar(stat="identity", position=ggplot2::position_dodge(.9)) +
  ggplot2::xlab('') + ggplot2::ylab(' ?? Accuracy') +
  ggplot2::coord_flip() + #  scale_y_continuous(limits=c(0, 0.8)) +
  ggplot2::scale_y_continuous(breaks=seq(0,1,0.2), limits=c(0, 1)) +
  ...
  ggtitle("Difference Split --> NoSplit Model") +
...
ggsave("int_vs_split_bars.png",grid_plot, width = 14, height = 6,dpi = 600
) ## width == 12 
