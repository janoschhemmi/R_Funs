...
## how many samples? 
smp_merged <- left_join(begin, int_tree_species_split, by = c("int_split" = "Baumart_int_age_density"))
smp_tally <- smp_merged %>% 
  group_by(int_split, TreeSpecies, Age) %>%
  tally()
smp_tally <- as.data.frame(smp_tally)
write.table(smp_tally, "P:/workspace/treespecies/_review/tables/smp_per_int_split_age_density.csv", sep = ";", dec = ".",
...
plot_rfcv_2018_2019 <- plot(y= tst_2018_2019_error_cv, x= tst_2018_2019_n_var, type = "line", 
                       ylab = "error rate", xlab = "n var", main = "Error rate Spec-model")

tst_2018_2019$predicted
saveRDS(tst_2018_2019, "A:/_ts_review/rfcv_2018_2019.rds")
#

## integer level 
col_int <- 7
df  <- (df_features_int_age)[,-8]
tst_2018_2019_int <- rfcv(trainx = df[,(col_int+1):ncol(df)], trainy = as.factor(df[,col_int]), 
                      scale = "log", step=0.6, cv.fold = 3)
tst_2018_2019_n_var    <- tst_2018_2019$n.var
tst_2018_2019_error_cv <- tst_2018_2019$error.cv
plot_rfcv_2018_2019 <- plot(y= tst_2018_2019_error_cv, x= tst_2018_2019_n_var, type = "line", 
...
## --> go with 475 bands 
imp_ <- row.names(importances[1:550,])
## subset df 
df_features_550 <- as.data.frame(cbind(df_old_bands[,1:8], 
                                       df_features[,colnames(df_features) %in% imp_]))

## 1 model df 550 feat
df <- df_features_550
col_int <- 8
model_spec_550 <- prandomForest(x = df[,(col_int+1):ncol(df)], y= as.factor(df[,col_int]),
                                classwt = tally_smp$anti )
...
df_env_ <- as.data.frame(cbind(df_old_bands[,1:8], 
                               df_env[,9:length(df_env)]))
df <- df_env
ncol(df_env[,9:ncol(df_env)])
col_int <- 8
model_env <- readRDS("P:/workspace/treespecies/models/01_model_paper/02_01_model_env_age_split.rds")

# 3 model env + spec 
df_spec_550_env <- cbind(df_features_550, df_env[,9:ncol(df_env)]) 
df <- df_spec_550_env
col_int <- 8
model_spec_550_env <- prandomForest(x = df[,(col_int+1):ncol(df)], y= as.factor(df[,col_int]),
                                    classwt = tally_smp$anti )
# 4 model env + spec + txt 
df_spec_550_env_txt <- cbind(df_features_550, df_env[,9:ncol(df_env)], df_txt[,9:ncol(df_txt)]) 
df <- df_spec_550_env_txt
ncol(df[,9:ncol(df)])
col_int <- 8
model_spec_550_env_txt <- prandomForest(x = df[,(col_int+1):ncol(df)], y= as.factor(df[,col_int]),
                                        classwt = tally_smp$anti )

## safe models 
setwd("P:/workspace/treespecies/_review/model/feature_reduction_env_txt/2018_2019/550/")
saveRDS(model_spec_550,"01_model_spec_550.rds")
saveRDS(model_env,"02_model_env.rds")
saveRDS(model_spec_550_env,"03_model_spec_550_env.rds")
saveRDS(model_spec_550_env_txt,"04_model_spec_550_env_txt.rds")
...
imp_ <- row.names(importances[1:550,])
## subset df 
df_features_550_int_ <- as.data.frame(cbind(df_old_bands[,1:7], 
                                       df_features[,colnames(df_features) %in% imp_]))

## 1 model df 550_int_ feat
df <- df_features_550_int_
col_int <- 7
model_spec_550_int_ <- prandomForest(x = df[,(col_int+1):ncol(df)], y= as.factor(df[,col_int]),
...
# 3 model env + spec 
df_spec_550_int__env <- cbind(df_features_550_int_, df_env[,9:ncol(df_env)]) 
df <- df_spec_550_int__env
col_int <- 7
model_spec_550_int__env <- prandomForest(x = df[,(col_int+1):ncol(df)], y= as.factor(df[,col_int]),
                                    classwt = tally_smp_int$anti )
# 4 model env + spec + txt 
df_spec_550_int__env_txt <- cbind(df_features_550_int_, df_env[,9:ncol(df_env)], df_txt[,9:ncol(df_txt)]) 
df <- df_spec_550_int__env_txt
col_int <- 7
model_spec_550_int__env_txt <- prandomForest(x = df[,(col_int+1):ncol(df)], y= as.factor(df[,col_int]),
                                        classwt = tally_smp_int$anti )

## safe models 
setwd("P:/workspace/treespecies/_review/model/new_bands_int/")
saveRDS(model_spec_550_int_,"01_model_spec_550_int_.rds")
saveRDS(model_env,"02_model_env.rds")
saveRDS(model_spec_550_int__env,"03_model_spec_550_int__env.rds")
saveRDS(model_spec_550_int__env_txt,"04_model_spec_550_int__env_txt.rds")
