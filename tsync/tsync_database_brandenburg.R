...
sqlite_file <- file.path(path, 'tsync_brandsat_bb_fire.dat')

# plot list
plot_file <- file.path(path, 'tsync_fires_4_distance_to_all_center.csv')
...
move_blank_chips <- function(chip.path) {
  ...
  dc$plotid <- sapply(strsplit(dc$name, '_'), FUN=function(x) x[[2]])
  dc$image_year <- sapply(strsplit(dc$name, '_'), FUN=function(x) x[[3]])
